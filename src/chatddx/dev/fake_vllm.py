...
It counts usage as vLLM does. Asked for it (`stream_options.include_usage`),
it sends it after the answer, and with `continuous_usage_stats` on every
chunk too, as far as the answer has got. A client that hangs up mid-stream
is heard as vLLM hears it through uvicorn and Starlette, as the connection
closes rather than at the next write, and the request is aborted: nothing
more is generated, and the server says so. In-process, a stream the client
closes before its end is aborted alike.
"""
...
def _stream(body: dict[str, Any], reply: Reply) -> Iterator[tuple[str, int]]:
    """
    Each event of the reply, streamed, and the tokens generated by the time
    it is sent. Continuous usage comes only with the usage at the end, as
    vLLM's `should_include_usage` has it.
    """
    model = body.get("model")
...
class FakeTransport(httpx2.AsyncBaseTransport):
    """
    The fake vLLM in-process, keeping every request it was sent, and each it
    aborted: a stream the client closed before its end.
    """
    ...
    async def next_token(self, _generated: int, /) -> None:
        """The wait for the next token, so many sent: none, in-process."""


class _Streamed(httpx2.AsyncByteStream):
    """A reply streamed in-process, as it is read, and aborted if it is closed first."""
    ...
    async def __aiter__(self) -> AsyncIterator[bytes]:
        ...
        for i, (event, generated) in enumerate(events):
            ...
            # with its last event out, the reply is whole: a client stops
            # reading at [DONE], and closes it
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _next_token(self) -> bool:
        """
        Wait the time the next token takes, or answer False if the client
        hangs up first: heard from the socket, as uvicorn hears it for vLLM,
        ...
        try:
            # readable, and nothing to read: the client's end is closed
            return self.connection.recv(1, socket.MSG_PEEK) != b""
        except OSError:
            return False

    def _sent(self, data: bytes) -> bool:
        """Whether `data` went out, to a client still there."""
        try:
