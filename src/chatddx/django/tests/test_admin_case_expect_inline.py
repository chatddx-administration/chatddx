...
def _inline_post_data(case_post_data, output_type_id, payload, *, extra=1):
    ...
    data |= {
        ...
        f"{prefix}-0-output_type": str(output_type_id),
...
def test_expect_inline_add_then_edit_is_idempotent_and_versions(
    ...
    output_type_branch = OutputTypeBranchModel.objects.get(
        owner__name=owner.name, name="output_type-1"
    )
