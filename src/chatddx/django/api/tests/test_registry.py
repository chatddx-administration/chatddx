...
def test_every_entity_lists_the_branches_the_identity_can_use(alice: Client):
    for entity in ENTITY_NAMES:
        response = alice.get(f"/api/registry/{entity}")

        assert response.status_code == 200, (entity, response.content)
        assert response.json(), entity


def test_stacks_are_listed_with_what_they_hold_and_where_they_send(alice: Client):
    stacks = named(alice.get("/api/registry/stack").json())
...
def test_configurations_are_listed_with_their_variations(alice: Client):
    free_text = named(alice.get("/api/registry/configuration").json())["free-text"]
...
def test_cases_are_listed_by_name_and_kept_to_any_of_the_tags(alice: Client):
    def listed(query: str) -> list[str]:
        return list(named(alice.get(f"/api/registry/case{query}").json()))
...
def test_a_case_says_its_text_and_its_targets(alice: Client, recommit: Recommit):
    shown = alice.get("/api/registry/case/case-1").json()
    case = shown["branch"]
    ...
    recommit("case", "case-2", name="calm", owner="alice", targets={"warning": False})

    calm = alice.get("/api/registry/case/calm").json()

    assert calm["branch"]["details"]["targets"] == {"warning": False}


def test_a_branch_counts_your_runs_with_it_by_scorer(alice: Client, run: Run):
    ...
    runs = alice.get("/api/registry/case/case-1").json()["runs"]
    ...
    listed = alice.get("/api/registry/case/case-1/runs").json()
    ...
    scorer = alice.get("/api/registry/scorer/first_mention").json()
...
def test_what_isn_t_there_isn_t_found(alice: Client):
    missing = alice.get("/api/registry/case/nope")

    assert missing.status_code == 404
    assert missing.json() == {"detail": "no case 'nope' for alice"}
    assert alice.get("/api/registry/frobnicate").status_code == 404


def test_another_s_configuration_is_had_by_its_owner(alice: Client, recommit: Recommit):
    recommit(
        "configuration", "plan", name="bobs-plan", owner="bob", collaborators=["alice"]
    )

    assert "bobs-plan" not in named(alice.get("/api/registry/configuration").json())
    assert alice.get("/api/registry/configuration/bobs-plan").status_code == 404

    plan = alice.get("/api/registry/configuration/bobs-plan?owner=bob").json()["branch"]

    assert (plan["name"], plan["owner"]["name"]) == ("bobs-plan", "bob")
    assert "warning" in plan["trail"]["output"]["views"]


def test_a_branch_lists_its_versions_the_head_first(alice: Client):
    ...
    versions = alice.get("/api/registry/configuration/mine/versions").json()

    head = alice.get("/api/registry/configuration/mine").json()["branch"]
