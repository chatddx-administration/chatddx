...
def expect_payloads(branch: CaseBranchModel) -> list[str]:
    return [expect.target.payload for expect in branch.expects.all()]
...
async def test_case_1(inventory_fixture_bs: InventoryBranchSpec):
    ...
    assert case_1.expects[0].name == "expect-1-a"
    assert case_1.expects[0].target.payload == EXPECT_1_A
    assert case_1.expects[0].target.scorer.command == "scorer-a command"
...
async def test_expects_belong_to_the_owner_not_to_the_payload(
    ...
    assert [expect.target.payload for expect in mine.expects] == [
        EXPECT_1_A,
        EXPECT_1_B,
    ]
    assert [expect.target.payload for expect in theirs.expects] == [
        "another expectation"
    ]
...
async def test_a_new_case_version_snapshots_what_it_supersedes(
    ...
    assert [expect.target.payload for expect in canon.expects] == [
        EXPECT_1_A,
        EXPECT_1_B,
    ]
...
async def test_named_expects_reach_the_canon_of_an_unchanged_case(
    ...
    canon = await case_canon(owner.name)
    assert [expect.target.payload for expect in canon.expects] == [EXPECT_1_A]


@pytest.mark.asyncio
async def test_a_case_keeps_its_own_of_two_identical_expectations(
    inventory_fixture_commit: object,
    owner: IdentityModel,
):
    """
    Two cases that expect the same payload for the same scorer share one
    content-addressed trail, and must still carry an expectation each.
    """

    _ = await commit_async(
        trail=ExpectTrailSchema(
            payload=EXPECT_1_A,
    ...
    case_1 = await case_canon(owner.name)
    case_2 = await case_canon(owner.name, "case-2")

    # the expectations are the same content, so they are the same trail
    assert case_1.expects[0].target.id == case_2.expects[0].target.id

    assert [expect.name for expect in case_1.expects] == ["expect-1-a", "expect-1-b"]
    assert [expect.name for expect in case_2.expects] == ["case-2|scorer-a"]
