...
- **The outcome** is `completed`, `valid` or `invalid` (for structured
  ...
  whose model runs away is stopped as `run` stops it: its answer is scored
  all the same, the line ends with why it stopped, and the batch goes on.
