...
* Tests share one test inventory, `src/chatddx/data/test-inventory.toml`, and its targets, `test-targets.toml`, through the fixtures in `src/chatddx/conftest.py`: repo commits it with its own inventory functions, every other package seeds it with init-data (`provision`), and a test without a database reads it parsed (`test_inventory`).
